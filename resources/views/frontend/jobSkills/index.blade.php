        ...
        <div class="col-md-12">
            @can('job_skill_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.job-skills.create') }}">
                            ...
                            <tbody>
                                @foreach($jobSkills as $key => $jobSkill)
                                    <tr data-entry-id="{{ $jobSkill->id }}">
                                        ...
                                        <td>
                                            @can('job_skill_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.job-skills.show', $jobSkill->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('job_skill_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.job-skills.edit', $jobSkill->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('job_skill_delete')
                                                <form action="{{ route('frontend.job-skills.destroy', $jobSkill->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('job_skill_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.job-skills.massDestroy') }}",
