        ...
        <div class="col-md-12">
            ...
            <div class="card">
                <div class="card-header">
                    {{ trans('global.create') }} {{ trans('cruds.jobSkill.title_singular') }}
                </div>

                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.job-skills.store") }}" enctype="multipart/form-data">
                        @method('POST')
                        @csrf
                        <div class="form-group">
                            <label class="required" for="job_posting_id">{{ trans('cruds.jobSkill.fields.job_posting') }}</label>
                            <select class="form-control select2" name="job_posting_id" id="job_posting_id" required>
                                @foreach($job_postings as $id => $entry)
                                    <option value="{{ $id }}" {{ old('job_posting_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="skills" id="skills" value="{{ old('skills', '') }}" required>
