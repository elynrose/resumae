...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.my-resumes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label class="required" for="resume">{{ trans('cruds.myResume.fields.resume') }}</label>
                <textarea class="form-control {{ $errors->has('resume') ? 'is-invalid' : '' }}" name="resume" id="resume" required>{{ old('resume') }}</textarea>
