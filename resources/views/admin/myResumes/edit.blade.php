...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.myResume.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.my-resumes.update", [$myResume->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="resume">{{ trans('cruds.myResume.fields.resume') }}</label>
                <textarea class="form-control {{ $errors->has('resume') ? 'is-invalid' : '' }}" name="resume" id="resume" required>{{ old('resume', $myResume->resume) }}</textarea>
            ...
            <div class="form-group">
                ...
                <div class="needsclick dropzone {{ $errors->has('resume_pdf') ? 'is-invalid' : '' }}" id="resume_pdf-dropzone">
                </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('user') ? 'is-invalid' : '' }}" name="user_id" id="user_id" required>
                    @foreach($users as $id => $entry)
                        <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $myResume->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('user'))
                    <div class="invalid-feedback">
...
<script>
    ...
    url: '{{ route('admin.my-resumes.storeMedia') }}',
    maxFilesize: 10, // MB
    maxFiles: 1,
    addRemoveLinks: true,
    headers: {
