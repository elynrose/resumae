...
@can('job_posting_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.job-postings.create') }}">
                        ...
                        <tr data-entry-id="{{ $jobPosting->id }}">
                            ...
                            <td>
                                @can('job_posting_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.job-postings.show', $jobPosting->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('job_posting_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.job-postings.edit', $jobPosting->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('job_posting_delete')
                                    <form action="{{ route('admin.job-postings.destroy', $jobPosting->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('job_posting_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.job-postings.massDestroy') }}",
